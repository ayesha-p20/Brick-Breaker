#BRICK - BREAKER
#RUN THIS FILE WITH ALL PARTS EQUATED TO 'FALSE' TO PLAY THE GAME.
import time
from bick_breaker_library import *
...
def setupBricks(lines_of_words, window, pixel_btwn_layers, pixel_per_char,brick_height, pixel_btwn_word, color_palette): #function to create bricks based on word length
   bricks = []
...
def showAndWaitStartClick(window, start_btn, start_text):  #check if 'Start' button is clicked
...
def checkTermination(window, ball, bricks, life):         #function to calculate 'LIFE'
...
def moveBall(ball, window, bricks, paddle):
   ...
   if status > 0:       #if the ball hits a brick or wall, change x-coordinate of its direction
      ball.setDirectionSpeed([-tmp_direction[0], 0])
      ball.moveIt()
      ball.setDirectionSpeed(tmp_direction)
      ball.reverseX()
      ball.moveIt()
      if status == 2:       #make the brick that was hit disappear
   ...
   ball.moveIt()
   status, brick = isCollide(ball, window, bricks, paddle)
   if status > 0:       #if the ball hits a brick or wall, change y-coordinate of its direction
      ball.setDirectionSpeed([0, -tmp_direction[1]])
      ball.moveIt()
      ball.setDirectionSpeed(tmp_direction)
      ball.reverseY()
...
def main():
   ...
   life_max = life
   while True:          #loop to determine status of game based on current value of 'LIFE'
      ...
      if new_life == 0:
         ...
         ball.setRandomDirectionSpeed(min_speed=min(0.85*life_max/life, 2.0), max_speed=max(1.0*life_max/life, 5.0))
