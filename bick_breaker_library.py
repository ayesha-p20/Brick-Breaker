...
class Brick:
    ...
    def getScore(self):  #scoring scale: a=0, b=1, c=2,...
        sum = 0
        for ch in self.text:
            if ch.lower():
                sum = sum + ord(ch)-97
...
class Ball:
    ...
    def resetToPaddle(self, paddle):     #place the ball in the center of the paddle 
        p = paddle.getSurfaceCenter()
    ...
    def checkHitWindow(self, window): #function to check if balls hits upper, left or right boundary of window in which case it is bounced back
    ...
    def checkHit(self, rectangle):         #function to check which portion of the paddle is hit by the ball
        ...
        if dist_x > (w/2 + rad) or dist_y > (h/2 + rad): return False     #if the ball does not touch the paddle, return FALSE  
        if dist_x <= (w/2 + rad) or dist_y <= (h/2 + rad): return True    
                                                                          #if the ball hits one of the corners of the paddle, return TRUE
...
def makeLifeStatic(window, life_input): #function to convert 'LIFE', which is entered by user in string form, to int and display it
...
def updateScore(score_offset, score_num_text): #score_offset is the score to be added to existing score, score_num_text is a text widget that displays the score in a string
